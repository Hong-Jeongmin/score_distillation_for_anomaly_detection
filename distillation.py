...
def measure_auroc(args, model, test_loader):
    ...
    df.to_csv(f'results/{args.dataset}_{args.model_name}_time.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher', default="CSI", type=str, choices=["CSI", "PANDA", "MSCL"])
    ...
    if not os.path.isdir(f'logs/{model_name}'):
    ...
    if args.mode == 'train':
        for epoch in range(1, args.epochs+1):
            ...
            writer.add_scalar(f'/{args.teacher}/{args.label}', loss, epoch)
            scheduler.step()
