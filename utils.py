...
def test_csi_score(x, model, features):
    axis, weight_sim, weight_shi = features["axis"], features["weight_sim"], features["weight_shi"]
    shift_trans = Rotation().to(device)
    layers = ('simclr', 'shift')
    feats_dict= {layer:[] for layer in layers}
    score=0
    for shi in range(4):
        x_t = shift_trans(x, shi)
        _, output_aux = model(x_t, simclr=True, shift=True)
        f_sim = output_aux['simclr'].flatten()
        f_shi = output_aux['shift'].flatten()
        
        neighbor = (f_sim * axis[shi]).sum(dim=1).max()
        score += neighbor.item() * weight_sim[shi]
        score += f_shi[shi].item() * weight_shi[shi]
    score = score/4
    return score
